...
from providers import AI_PROVIDERS, BROWSE_GROUPS, ANTHROPIC_GROUPS
...
class AIClient:
    ...
    async def get_group_models(self, group: str, api_key: str) -> List[Dict[str, str]]:
        from config import OR_CACHE_TTL
    ...
    async def chat(self, provider: Dict[str, Any], messages: list, api_key: str, system_prompt: str) -> str:
        ...
        if is_anthropic:
            ...
            body = {"model": provider["model_id"], "system": system_prompt,
                    "messages": [m for m in messages if m.get("role") != "system"], "max_tokens": 2048}
        else:
            headers = {"Authorization": f"Bearer {api_key}"}
            body = {"model": provider["model_id"], "messages": messages, "max_tokens": 2048}
